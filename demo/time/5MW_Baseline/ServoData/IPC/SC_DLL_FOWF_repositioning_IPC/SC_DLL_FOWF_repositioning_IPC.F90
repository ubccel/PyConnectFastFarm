...
!GCC$ ATTRIBUTES DLLEXPORT :: sc_calcOutputs
   ...
   REAL(C_DOUBLE) :: yawRef(2*nTurbines)
   REAL(C_DOUBLE) :: pitchRef(2*nTurbines)
   REAL(C_DOUBLE) :: torqueRef(2*nTurbines)
   REAL(C_DOUBLE) :: ipcRef(4*nTurbines)
   ...
   integer message
   integer yaw_tag, pitch_tag, torque_tag, com_tag, measures_tag, ipc_tag
   ...
   ! Initialize command arrays to zero (prevent garbage before MPI is up)
   yawRef = 0.0d0
   pitchRef = 0.0d0
   torqueRef = 0.0d0
   ipcRef = 0.0d0
   ...
   if (initialized) then
      call MPI_RECV(yawRef, 2*nTurbines, MPI_DOUBLE, command_process_rank, yaw_tag, parent_comm, statut, ierror)
      call MPI_RECV(pitchRef, 2*nTurbines, MPI_DOUBLE, command_process_rank, pitch_tag, parent_comm, statut, ierror)
      call MPI_RECV(torqueRef, 2*nTurbines, MPI_DOUBLE, command_process_rank, torque_tag, parent_comm, statut, ierror)
      call MPI_RECV(ipcRef, 4*nTurbines, MPI_DOUBLE, command_process_rank, ipc_tag, parent_comm, statut, ierror)
   ...
   do j = 1, nTurbines
      from_SC((j-1)*NumSC2Ctrl+1) = yawRef(2*j-1)     ! per-turbine yaw activation flag
      from_SC((j-1)*NumSC2Ctrl+2) = pitchRef(2*j-1)   ! per-turbine pitch activation flag
      from_SC((j-1)*NumSC2Ctrl+3) = torqueRef(2*j-1)  ! per-turbine torque activation flag
      from_SC((j-1)*NumSC2Ctrl+4) = yawRef(2*j)       ! per-turbine yaw ref
      from_SC((j-1)*NumSC2Ctrl+5) = pitchRef(2*j)     ! per-turbine pitch ref
      from_SC((j-1)*NumSC2Ctrl+6) = torqueRef(2*j)    ! per-turbine torque ref
      ! IPC: per-turbine activation flag + 3 per-blade offsets
      from_SC((j-1)*NumSC2Ctrl+7)  = ipcRef(4*j-3)    ! per-turbine IPC activation flag
      from_SC((j-1)*NumSC2Ctrl+8)  = ipcRef(4*j-2)    ! blade 1 IPC offset
      from_SC((j-1)*NumSC2Ctrl+9)  = ipcRef(4*j-1)    ! blade 2 IPC offset
      from_SC((j-1)*NumSC2Ctrl+10) = ipcRef(4*j)      ! blade 3 IPC offset
   end do
