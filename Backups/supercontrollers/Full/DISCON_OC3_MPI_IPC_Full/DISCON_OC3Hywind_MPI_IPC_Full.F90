...
REAL(C_FLOAT),          INTENT(IN   ) :: from_SC_Glob(*)                ! DATA from the supercontroller (global)
REAL(C_FLOAT),          INTENT(IN   ) :: from_SC     (*)                ! DATA from the supercontroller (turbine-specific)
...
REAL(4)                      :: YawRef                                           ! Yaw reference from supercontroller, rad
REAL(4)                      :: PitchRef                                         ! Pitch reference from supercontroller, rad
REAL(4)                      :: TorqueRef                                        ! Torque reference from supercontroller, N-m
REAL(4)                      :: YawAngle                                         ! Measured nacelle yaw angle from North, rad
LOGICAL(1)                   :: YawExternalCommand    = .FALSE.                  ! Flag: use supercontroller yaw command
LOGICAL(1)                   :: PitchExternalCommand  = .FALSE.                  ! Flag: use supercontroller pitch command
LOGICAL(1)                   :: TorqueExternalCommand = .FALSE.                  ! Flag: use supercontroller torque command
LOGICAL(1)                   :: IPCExternalCommand    = .FALSE.                  ! Flag: use supercontroller IPC command
REAL(4)                      :: IPCRef(3)                                        ! IPC per-blade pitch offsets from supercontroller, rad
...
IF ( iStatus == 0 )  THEN  ! .TRUE. if we're on the first call to the DLL
   ...
   LastFiltPitCom = BlPitch                     ! Initialize filtered pitch command with current blade pitch
...
!=======================================================================
   ...
   PitchRef  = from_SC(5)
   TorqueRef = from_SC(6)
   ! IPC: individual pitch control offsets (superimposed on CPC)
   IPCExternalCommand = from_SC(7) == 1.0
   IPCRef(1) = from_SC(8)
   IPCRef(2) = from_SC(9)
   IPCRef(3) = from_SC(10)
   ...
   ! Yaw rate command from supercontroller
   IF (YawExternalCommand) THEN
      avrSWAP(48) = 1.0*(YawRef - YawAngle)   ! proportional yaw rate command
   ENDIF

   ! Send turbine measurements to supercontroller (69 variables)
   to_SC(1)  = avrSWAP(21)                    ! rotor speed, rad/s
   to_SC(2)  = avrSWAP(20)                    ! generator speed, rad/s
   to_SC(3)  = avrSWAP(15)                    ! measured electrical power, W
   to_SC(4)  = avrSWAP(27)                    ! hub wind speed, m/s
   to_SC(5)  = YawAngle                       ! measured nacelle yaw angle, rad
   to_SC(6)  = avrSWAP(4)                     ! blade 1 pitch angle, rad
   to_SC(7)  = avrSWAP(33)                    ! blade 2 pitch angle, rad
   to_SC(8)  = avrSWAP(34)                    ! blade 3 pitch angle, rad
   to_SC(9)  = avrSWAP(30)                    ! blade 1 root out-of-plane bending moment, N-m
   to_SC(10) = avrSWAP(31)                    ! blade 2 root out-of-plane bending moment, N-m
   to_SC(11) = avrSWAP(32)                    ! blade 3 root out-of-plane bending moment, N-m
   to_SC(12) = avrSWAP(69)                    ! blade 1 root in-plane bending moment, N-m
   to_SC(13) = avrSWAP(70)                    ! blade 2 root in-plane bending moment, N-m
   to_SC(14) = avrSWAP(71)                    ! blade 3 root in-plane bending moment, N-m
   to_SC(15) = avrSWAP(53)                    ! tower top fore-aft acceleration, m/s^2
   to_SC(16) = avrSWAP(54)                    ! tower top side-side acceleration, m/s^2
   to_SC(17) = avrSWAP(2)                     ! current time, s
   to_SC(18) = avrSWAP(60)                    ! LSS azimuth angle, rad
   to_SC(19) = avrSWAP(23)                    ! HSS/generator torque, N-m
   to_SC(20) = avrSWAP(27)                    ! wind speed at hub height, m/s
   to_SC(21) = avrSWAP(24) + avrSWAP(37)     ! wind direction from North, rad
   to_SC(22) = avrSWAP(1001)                  ! platform surge TDX, m
   to_SC(23) = avrSWAP(1002)                  ! platform sway TDY, m
   to_SC(24) = avrSWAP(1003)                  ! platform heave TDZ, m
   to_SC(25) = avrSWAP(1004)                  ! platform roll RDX, rad
   to_SC(26) = avrSWAP(1005)                  ! platform pitch RDY, rad
   to_SC(27) = avrSWAP(1006)                  ! platform yaw RDZ, rad
    ! --- Extended measurements ---
    to_SC(28) = avrSWAP(14)                    ! measured shaft power RotPwr, W
    to_SC(29) = avrSWAP(73)                    ! rotating hub My LSSTipMya, N-m
    to_SC(30) = avrSWAP(74)                    ! rotating hub Mz LSSTipMza, N-m
    to_SC(31) = avrSWAP(75)                    ! fixed hub My LSSTipMys, N-m
    to_SC(32) = avrSWAP(76)                    ! fixed hub Mz LSSTipMzs, N-m
    to_SC(33) = avrSWAP(77)                    ! yaw bearing My YawBrMyn, N-m
    to_SC(34) = avrSWAP(78)                    ! yaw bearing Mz YawBrMzn, N-m
    to_SC(35) = avrSWAP(82)                    ! nacelle roll accel NcIMURAxs, rad/s^2
    to_SC(36) = avrSWAP(83)                    ! nacelle nodding accel NcIMURAys, rad/s^2
    to_SC(37) = avrSWAP(84)                    ! nacelle yaw accel NcIMURAzs, rad/s^2
    to_SC(38) = avrSWAP(95)                    ! air density AirDens, kg/m^3
    to_SC(39) = avrSWAP(96)                    ! rotor-avg wind speed AvgWindSpeed, m/s
    to_SC(40) = avrSWAP(109)                   ! shaft torque LSSTipMxa, N-m
    to_SC(41) = avrSWAP(110)                   ! shaft thrust LSShftFxa, N
    to_SC(42) = avrSWAP(111)                   ! shaft force Y LSShftFys, N
    to_SC(43) = avrSWAP(112)                   ! shaft force Z LSShftFzs, N
    to_SC(44) = avrSWAP(1007)                  ! platform velocity TVX, m/s
    to_SC(45) = avrSWAP(1008)                  ! platform velocity TVY, m/s
    to_SC(46) = avrSWAP(1009)                  ! platform velocity TVZ, m/s
    to_SC(47) = avrSWAP(1010)                  ! platform velocity RVX, rad/s
    to_SC(48) = avrSWAP(1011)                  ! platform velocity RVY, rad/s
    to_SC(49) = avrSWAP(1012)                  ! platform velocity RVZ, rad/s
    to_SC(50) = avrSWAP(1013)                  ! platform accel TAX, m/s^2
    to_SC(51) = avrSWAP(1014)                  ! platform accel TAY, m/s^2
    to_SC(52) = avrSWAP(1015)                  ! platform accel TAZ, m/s^2
    to_SC(53) = avrSWAP(1016)                  ! platform accel RAX, rad/s^2
    to_SC(54) = avrSWAP(1017)                  ! platform accel RAY, rad/s^2
    to_SC(55) = avrSWAP(1018)                  ! platform accel RAZ, rad/s^2
    ! --- Config parameters ---
    to_SC(56) = avrSWAP(3)                     ! DLL_DT communication interval, s
    to_SC(57) = avrSWAP(6)                     ! Pitch_Min, rad
    to_SC(58) = avrSWAP(7)                     ! Pitch_Max, rad
    to_SC(59) = avrSWAP(8)                     ! PitchRate_Min, rad/s
    to_SC(60) = avrSWAP(9)                     ! PitchRate_Max, rad/s
    to_SC(61) = avrSWAP(13)                    ! GenPwr_Dem demanded power, W
    to_SC(62) = avrSWAP(16)                    ! Gain_OM optimal mode gain, Nm/(rad/s)^2
    to_SC(63) = avrSWAP(17)                    ! GenSpd_MinOM, rad/s
    to_SC(64) = avrSWAP(18)                    ! GenSpd_MaxOM, rad/s
    to_SC(65) = avrSWAP(19)                    ! GenSpd_Dem above rated, rad/s
    to_SC(66) = avrSWAP(22)                    ! GenTrq_Dem above rated, N-m
    to_SC(67) = avrSWAP(28)                    ! Ptch_Cntrl (0=collective, 1=individual)
    to_SC(68) = avrSWAP(29)                    ! Yaw_Cntrl (0=rate, 1=torque)
    to_SC(69) = avrSWAP(61)                    ! NumBl number of blades

   ! Initialize filtered pitch for this time step
   LastFiltPitCom = PitCom
...
!=======================================================================
   ...
   ! Compute the generator torque, which depends on which region we are in:
      ...
      IF (.NOT. TorqueExternalCommand) THEN
         ! Use native OC3Hywind torque control

         IF ( (   GenSpeedF >= VS_RtGnSp ) .OR. (  PitCom(1) >= VS_Rgn3MP ) )  THEN ! We are in region 3 - constant torque (Hywind modification)
            GenTrq = VS_RtPwr/PC_RefSpd   ! constant torque in Region 3 (not constant power) for Hywind spar
   ...
   ! Saturate the commanded torque using the torque rate limit:
      ...
      ELSE
         ! Use supercontroller torque command
         GenTrq = TorqueRef
...
!=======================================================================
   ...
   ! Apply supercontroller pitch command if active (take max of SC ref and native command,
   !   then apply exponential smoothing filter):

      IF (PitchExternalCommand) THEN

         PitchRef = MAX(PitchRef, PitComT)   ! use the larger of SC reference and native pitch command
   ...
   ! Superimpose IPC offsets at the output stage only.
   ! Do NOT modify PitCom (SAVE'd) — it feeds back into the rate limiter next step.
   IF (IPCExternalCommand) THEN
      avrSWAP(42) = avrSWAP(42) + IPCRef(1)
      avrSWAP(43) = avrSWAP(43) + IPCRef(2)
      avrSWAP(44) = avrSWAP(44) + IPCRef(3)
   END IF
